...
const datasets: Dataset[] = [
  {
    ...
    headers: ["id", "nombre", "correo", "rol", "estado", "plan", "creado", "último acceso"],
  ...
  },
  {
    ...
    headers: ["id", "cliente", "correo", "importe", "artículos", "estado", "método", "fecha"],
            ...
            <div key={dataset.id} className="surface-hairline rounded-lg p-5">
              ...
              <div className="mt-4 flex items-center gap-2 border-t pt-4">
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => downloadCsv(`${dataset.id}.csv`, dataset.headers, dataset.rows)}
                >
                  <FileText className="h-3.5 w-3.5" aria-hidden="true" /> CSV
                </Button>
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() =>
                    downloadExcel(`${dataset.id}.xls`, dataset.name, dataset.headers, dataset.rows)
                  }
                >
            ...
            <Card key={report.id} className="surface-hairline rounded-lg">
                ...
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <CardDescription className="mt-1">
                      {typeLabel[report.type]} · {report.format.toUpperCase()}
                    </CardDescription>
                  </div>
                  <Switch
                    ...
                    onCheckedChange={(checked) =>
                      setReports((current) =>
                        current.map((item) =>
                          item.id === report.id ? { ...item, enabled: checked } : item,
                        ),
                      )
                    }
              ...
              <CardContent>
                <div className="space-y-1.5 text-xs text-muted-foreground">
                  <p>
                    <span className="mono-label text-[10px]">última ejecución</span> ·{" "}
                    {formatDateTime(report.lastRun)}
                  </p>
                  <p>
                    <span className="mono-label text-[10px]">próxima</span> ·{" "}
                    {formatDateTime(report.nextRun)}
                  </p>
                  <p className="truncate">
                    <span className="mono-label text-[10px]">destinatarios</span> ·{" "}
                    {report.recipients.join(", ")}
                  </p>
                </div>
                <div className="mt-4 flex items-center justify-between border-t pt-4">
                  <Badge variant={report.enabled ? "success" : "secondary"} className="mono-label text-[10px]">
                    {report.enabled ? "activo" : "pausado"}
                  </Badge>
                  <Button variant="ghost" size="sm">
                    <Play className="h-3.5 w-3.5" aria-hidden="true" /> Generar ahora
                  </Button>
                </div>
              </CardContent>
            </Card>
