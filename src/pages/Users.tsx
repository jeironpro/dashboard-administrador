...
export function Users() {
  ...
  function saveUser() {
    ...
    if (editing) {
      setUsers((current) =>
        current.map((user) => (user.id === editing.id ? { ...user, ...form } : user)),
      );
    } else {
      const newUser: User = {
  ...
  function toggleSuspend(user: User) {
    setUsers((current) =>
      current.map((item) =>
        item.id === user.id
          ? { ...item, status: item.status === "suspendido" ? "activo" : "suspendido" }
          : item,
      ),
    );
  }
                      ...
                      <DropdownMenuContent align="end" className="w-48">
                        ...
                        <DropdownMenuItem onClick={() => toggleSuspend(user)}>
                          {user.status === "suspendido" ? (
                            ...
                            <>
                              <Ban className="h-4 w-4" aria-hidden="true" /> Suspender
                            </>
                          )}
                        </DropdownMenuItem>
                        <DropdownMenuItem
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              {editing ? editing.email : "Completa los datos del nuevo usuario."}
            </DialogDescription>
          </DialogHeader>
